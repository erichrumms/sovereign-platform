...
**HEAD as of this Brief:** `6b0c707` (July 18), verified against
`origin/main` directly. All prior unconfirmed-commit caveats are closed:
`1e08b9c` (Agent Identity Standard Session Findings Note + verification
scripts), `3352780`/`c6514b5` (SBOM audit placement, later superseded),
`0febc35`/`671f063` (AGENT_REFERENCE manifest corrections), `33c5455`
(v1.40 manifest), `6b0c707` (repo SBOM artifact removal per Lesson 13).
...
AGENT_REFERENCE v2.0 trigger discipline.
...
| Place docs/18 v1.1 | **DECIDED July 18: process it.** Repo's stale v1.0 to be replaced by v1.1 (417 lines, `844c23f2…`) | In progress this cycle |
...
| AGENT_REFERENCE v2.0 counter value | v2.0 was assigned as a stated assumption; prior internal counter value not recovered | Renumber if the old value is known |
...
| `lens-orientation` table correction | Findings Note documents it; the registry table itself not yet edited | Governance Agent task, low risk |
...
| Dispose superseded iCloud SBOMs | v1.38, v1.39 → For Disposal per folder rules | Project Principal, leisure |

**UNCHANGED, carried forward:** exhibit-drafter validation failure;
Agent_Identity_Standard TT Status fields + terminology fix; PPBE
`max_tokens` host wiring; `docs/16` WF-12 corrections; root cleanup (~37
stale Brief versions); WF-2, WF-6, WF-8 governance decisions;
ARIA-EXPORT-GD and F-2 (both surfaced in SBOM history, still open).
...
| **v1.47** | **July 18** | **Walkthrough F + Session 38 absorbed; orientation pass: test count re-derived (1923), SBOM lineage completed to v1.40 and repo brought Lesson-13 compliant, two AGENT_REFERENCE lineages documented and v2.0 placed, Lesson 13 located, docs/18 staleness found; Lesson 23 added** |
...
| Place docs/18 v1.1 | Nothing — quick |

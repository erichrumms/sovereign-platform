#!/bin/bash
# SOVEREIGN — Session 126 Context Gather Script
...
OUT="/tmp/sovereign_session126_context.txt"
> "$OUT"
...
echo "SOVEREIGN — Session 126 context gather   $(date '+%Y-%m-%d %H:%M')"
echo "HEAD: $(git rev-parse --short HEAD)"
echo "Expected prior close: Session 125, terminal HEAD 5864d99 — confirm below."
echo
...
if command -v pbcopy >/dev/null 2>&1; then
  ...
  echo "then paste the Session 126 opening prompt (PPBE Exhibit F-51 closure,"
  echo "option b) as the second."
else

...
**Changed this version — the biggest single jump in this document's history.**
...
| **SCRIBE's two unwritten prompts** *(new, OPEN — not previously known)* | The July 19 assessment found `framing_system.md` and `synthesis_system.md` registered with no file on disk under any naming convention, and — unlike `lens-orientation`'s explicit, documented deferral — no record anywhere stating whether this is deliberate or an oversight. **Open decision, tracked in Part VI: formally defer, or schedule authoring.** |
| **Cross-module data-access architecture** *(new, OPEN — flagged as top sequencing priority)* | Session 40 correctly Hard-Stopped an attempt to have VIGIL's obligation-approval brief pull live context from APEX — no cross-module data path exists anywhere in the codebase, and forcing one would have meant either a fragile direct import or duplicated logic. The July 19 assessment independently named this the platform's clearest remaining sequencing risk: further consolidation ideas (a unified reviewer queue, in-context program summaries) all depend on the same underlying fix. **Needs a real spec before any build session touches it — tracked in Part VI, not yet scheduled.** |
| **Governance document placement** *(new, OPEN — mechanical, not a decision)* | Two documents corrected July 19 (`Agent_Identity_Standard.md`'s agent-count self-contradiction; `AGENT_REFERENCE.md` v3.1's new ARIA pattern documentation) are built and verified but not yet committed to the repo, iCloud, or (for `AGENT_REFERENCE.md`) project knowledge. No decision required — just needs doing. See Part VIII. |
...
| **Governance doc corrections** | ~~`Agent_Identity_Standard.md` agent-count fix; `AGENT_REFERENCE.md` v3.1 ARIA-pattern documentation~~ — built and verified, **not yet placed.** | Governance Agent | **Immediate next step — see Part VIII** |
| **Cross-module data-access architecture** | Real spec needed before any build session touches this — Session 40's Hard Stop, independently named the top sequencing risk by the End-to-End Assessment | Governance Agent, spec first | Not yet scheduled |
| **SCRIBE prompt decision** | Formally defer `framing_system.md`/`synthesis_system.md`, or schedule authoring | Project Principal decision | Not yet decided |
| **Rehearsal** | Full five-item demo script, both workflow layers, dry-run — including the role-based access story (Part I, item 2) | Project Principal + Governance Agent | Governance docs placed; SCRIBE prompt decision made |
...
remaining repo-root Brief version cleanup), live-testing the six DEV-persona
roles never yet seen on screen (ANALYST, COMPLIANCE_OFFICER, AGENT_OPERATOR,
INDEPENDENT_REVIEWER, READ_ONLY, PLATFORM_ADMIN — code-verified only so far).
...
|---|---|
| 27–43 | All done |
| Cross-module architecture build session | A real spec, written before the session opens — not an improvised opening prompt (Session 40's Hard Stop is exactly why) |
| Rehearsal | Governance docs placed; SCRIBE prompt question resolved either direction |
...
| **SCRIBE's two unwritten prompts** *(new)* | Not the current sequence, but genuinely undecided — unlike `lens-orientation` this has no documented deferral | Formally defer (update the standard), or schedule real authoring — either is fine, silence isn't |
| **Cross-module data-access architecture** *(new — highest leverage item in this table)* | **Every future consolidation idea** — a unified reviewer queue, in-context program summaries on approval screens, anything requiring one module to see another's data | This is the plan's actual critical path now. Needs a real written spec before any build session, per Session 40's own Hard Stop |
...
- [ ] **Place `Agent_Identity_Standard.md` and `AGENT_REFERENCE.md`** (repo, iCloud, and project knowledge for the latter) — mechanical, immediate next step, no decision required
- [ ] **Decide the SCRIBE prompt question** — formally defer, or schedule authoring
- [ ] Live-test the six DEV-persona roles never yet seen on screen, and re-confirm Session 42/43's changes live rather than by code review alone (Part IV)
- [ ] Rehearse the full five-item demo script, including the new role-access story beat
...
- [ ] Decide whether to include Part X (Local LLM) in the live demo narrative or hold it strictly as Q&A backup — either is reasonable, this plan defaults to Q&A-only unless told otherwise
...
- [x] ~~Write the Walkthrough F scenario script and opening materials~~ — DONE, both the original and repeat-pass scripts
- [x] ~~Reconcile Sessions 35 through 43's closes~~ — DONE, each independently re-verified against the actual repo, not taken from any recap
- [x] ~~Full terminology scrub across all governing documents~~ — DONE
- [x] ~~Correct `Agent_Identity_Standard.md`'s agent-count self-contradiction~~ — DONE, built and verified; placement pending (Project Principal, above)
- [x] ~~Document the ARIA per-tab pattern in `AGENT_REFERENCE.md`~~ — DONE, v3.1; placement pending
- [x] ~~Produce the July 19 System Prompt refresh (v32)~~ — DONE
- [ ] **Write a real spec for the cross-module data-access architecture** — the plan's actual next real deliverable; do not let a build session open on an improvised version of this
- [ ] Keep Part IX current as tweaks occur
- [ ] Resolve `module-lens` orientation prompt (missing on disk, but formally deferred and documented — low urgency) and VIGIL triage prompt path drift — both still open, both harmless
- [ ] Run the `git log -p` check on `Agent_Identity_Standard.md`'s PPBE Status-field discrepancy before correcting it
- [ ] Guide the live re-verification of Session 42/43's changes and the six untested DEV-persona roles (Part IV)
- [ ] Draft the Home page dashboard design and the SCRIBE visual redesign content decisions — both deliberately deferred, neither yet spec'd

**Build Agent:**
- [x] ~~Sessions 27 through 37~~ — all complete
- [x] ~~Author all four PPBE prompts~~ — done, Session 32
- [x] ~~Session 39 — Walkthrough F repeat-pass remediation~~ — done
- [x] ~~Session 40 — DR-2, DR-1 Tier 1; correctly Hard-Stopped DR-3's near-term move~~ — done
- [x] ~~Session 41 — GD-22 role-based access matrix, all four deliverables~~ — done
- [x] ~~Session 42 — module labels/hover info, WF-25–28~~ — done
- [x] ~~End-to-End Verification + Technical Assessment~~ — done, advisory only, no documents self-edited
- [x] ~~Session 43 — WF-14 fix at all 8 locations + test-inventory gap closed + repo cleanup + verify-script refresh~~ — done
- [ ] Resolve `PROMPT-REGISTRY-DRIFT` when convenient — the underscore/unversioned vs. hyphen/versioned prompt filename mismatch, confirmed harmless, still cosmetically open
- [ ] `F-2` — low priority, not demo-path, resolve whenever convenient
- [ ] `F-3` — `vigil-approval-agent`'s unexplained prompt file, flagged, never traced
...
- [ ] Build zero-to-some test coverage for `sovereign-shell` (nav, DevPersonaToggle, PlatformHome) — flagged by the July 19 assessment as thin relative to its risk; not yet scoped into any session
...
- [ ] Cross-module data-access architecture build — **blocked on the Governance Agent's spec, above; do not open this as an improvised session**
...
| **July 19, 2026** | **v3.4 — the largest single update in this document's history.** Walkthrough F ran twice, live (original run + a required repeat pass), closing with 24 total findings (WF-1–WF-24), all fixed and re-verified. GD-22 (role-based access matrix) designed, approved, and built — corrected a real, previously-invisible gap where no non-admin role could reach any module. Sessions 39-43 all closed. The first genuine post-milestone technical assessment ran per this plan's own Part IV standard, independently confirming test suites and finding two new real issues (WF-14, a precisely-diagnosed contrast failure; an agent-count self-contradiction) invisible to prior checks. Two governance documents corrected as a direct result (`Agent_Identity_Standard.md`, `AGENT_REFERENCE.md` v3.1) — built, not yet placed. **A new process risk was found and fixed:** two sessions closed on a chat recap rather than an actual push; the Close Protocol is now mandatory in every opening prompt. **New open items, not previously known:** two unwritten SCRIBE prompts with no documented deferral; the cross-module data-access architecture question, now the plan's clearest sequencing risk. Part X unchanged. | Sessions 39-43, a governance reconciliation pass, and the first Part-IV-standard technical assessment, relayed and independently re-verified by the Governance Agent across every step, not taken from any single recap |
...
*SOVEREIGN Platform — Strategic Plan to CTO Demo · v3.4 · July 19, 2026*
*Pre-Decisional · Internal Working Document*
*File to: git `docs/`*
*Companion: `SOVEREIGN_EndToEnd_Assessment_20260719.md`*

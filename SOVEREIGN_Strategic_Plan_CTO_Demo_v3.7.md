...
**Changed this version:** eight more sessions closed (54–61) since v3.6's four. The
documentation-currency pass v3.6 flagged as "in progress" is now largely complete — this
document, the Integration Brief, the Agent-to-Agent Briefing, the New Conversation Handoff,
`AGENT_REFERENCE.md`, `docs/22`, `docs/28`, `docs/29`, `docs/30`, the Role Access Matrix, and
`docs/18` were all rewritten or updated the same day as this version. **A real governance
conversation (`docs/29`) resolved the largest open architecture question left from v3.6** —
APEX's original program dataset and PPBE's synthetic one are not the same entities under two ID
schemes, decided not to force-merge them, and built a native PPBE Program Detail view instead
(Sessions 57). **GD-28 shipped** (Session 58): the Logger's dormant `getEntries()` method finally
exposed through the shell contract, and a real Activity & Decisions tab built in the Reviewer's
Workspace — the platform can now show a reviewer their own decision history, session-scoped, for
the first time. **A genuine platform-wide assessment ran** (Session 60) — the first
comprehensive, code-level reliability/efficiency/security pass across all eleven modules, Home,
and the Workspace, explicitly and repeatedly stated as *not* a substitute for a live Walkthrough,
since no browser automation exists on this machine. It found the platform's infrastructure
genuinely healthy and one real systemic finding: the session-state-resurrection bug class,
already fixed twice (VIGIL approvals, SCRIBE), existed unfixed in four more places, plus a
latent, previously-invisible ordering dependency between that bug family and any future
Home-return navigation work. **Session 61 — the largest single session in this plan's
history — closed all of it**, in the exact order the dependency required: the root fix first
(real live subscription replacing mount-time seeding), then the four sibling applications, then,
only once safe, genuine Home-return navigation. Every claim independently re-verified against
the real repository, including a safety-critical duplicate-attestation guard checked twice before
being trusted. **Two real process lessons, now standing:** a session's own claim *about the
state of the repository* (not just about code) needs the same direct verification as a code
claim — two consecutive sessions got the same small file-location question wrong, two different
ways; and a genuine architectural ordering dependency can exist between two findings that share
no code and no obvious connection — confirmed real this cycle, not just a theoretical risk.
**The live Walkthrough repeat pass on Home Dashboard is now the single oldest and most
load-bearing open item in the platform's entire history** — open since Session 54, and Session
61's own close names a specific reason it matters more now: the sequence "enter a module, decide
an item, return Home, re-enter" exercises this cycle's two biggest fixes together and has never
been seen in an actual browser.
**Changed in v3.6, for reference:**
...
| **Governance document placement (round 2)** | **COMPLETE.** `docs/24` and `docs/25` both confirmed placed. |
| **`docs/29` — WG-11/WG-8/WG-7 program-model decisions** *(new this version, DECIDED and BUILT)* | Session 57. PPBE and the World Model's original programs are not force-merged (real evidence checked: thematically similar but never-identical names, no cross-reference anywhere). A native PPBE Program Detail view built instead, delivering WG-8 (per-program selector) as the same feature. Module Orientation given a real job (WG-7): live per-module status via `WorkQueueSurface`, plus clickable rows. |
| **GD-28 — Logger read exposure** *(new this version, DECIDED and BUILT)* | Session 58. Shell contract v1.22 → v1.23. `getEntries()` — a real method that already existed, unused, one layer below the public interface — finally exposed. The Activity & Decisions tab built in the Reviewer's Workspace: a reviewer's own decision history, session-scoped, for the first time anywhere in the platform. |
| **Session 60 — comprehensive R/E/S assessment** *(new this version, COMPLETE)* | The first platform-wide, code-level reliability/efficiency/security pass across all eleven modules, Home, and the Workspace. Explicitly not a Walkthrough substitute. Found the session-state-resurrection bug class in four more places, plus a real, confirmed ordering dependency (D3-9) between that family and any future Home-return fix. |
| **Session 61 — resurrection family + navigation, closed** *(new this version, COMPLETE)* | All seven of the Session 60 assessment's build findings, in the required order. The largest single session in this plan's history. Full detail: `docs/30`. |
...
| **Session 54** | ~~Walkthrough G Build 1: eager cross-module data population, VIGIL live expiry sweep, sidebar tooltip portal fix, APEX chart fixes, per-item dependency detail, VIGIL's first session-persistent store~~ — COMPLETE. | Build Agent build | Done |
| **Session 55** | ~~WG-15/16/17: SCRIBE sent-session store, Home expiry sweep, Workspace post-decision republish~~ — COMPLETE, but shipped with zero new tests and a test-count table that didn't match its own row sum. | Build Agent build | Done |
| **Session 56** | ~~Closed Session 55's gaps directly: 14 real tests, npm audit scope resolved definitively~~ — COMPLETE. | Build Agent build | Done |
| **`docs/29` (governance conversation)** | ~~Resolved the program-model question and Module Orientation's fate; decided GD-28's shape~~ — COMPLETE. | Governance Agent + Project Principal | Done |
| **Session 57** | ~~Native PPBE Program Detail view (WG-11+WG-8) + Module Orientation live status (WG-7)~~ — COMPLETE. | Build Agent build | Done |
| **Session 58 (GD-28)** | ~~Logger `getEntries()` exposed, shell contract v1.22→v1.23; Activity & Decisions tab built~~ — COMPLETE. One naming-convention slip in the Handoff, corrected via direct follow-on. | Build Agent build | Done |
| **Session 59** | ~~WG-6 demo-cosmetic fiscal-period padding; decision-note reason-code chips (VIGIL+ARIA); memory-contamination cleanup~~ — COMPLETE. | Build Agent build | Done |
| **Session 60** | ~~Comprehensive end-to-end R/E/S assessment, all eleven modules + Home + Workspace~~ — COMPLETE. Found the resurrection-family finding and the D3-9 ordering dependency. | Build Agent, advisory/findings only | Done |
| **Session 61** | ~~The full resurrection family (5 stores) + Home-return navigation, all seven deliverables in the required order~~ — COMPLETE. The largest single session in this plan's history. | Build Agent build | Done |
| **Documentation-currency pass (round 2)** *(COMPLETE this version)* | Agent-to-Agent Briefing, Platform Integration Brief, New Conversation Handoff, `docs/18`, this document, `AGENT_REFERENCE.md`, `docs/22`/`28`/`29`/`30`, Role Access Matrix — all rewritten or updated the same day | Governance Agent | **Done** |
| **SBOM Registry** | v1.41 covers through Session 53. **Eight real session updates (54–61) exist but are not yet merged into a new registry version** — a genuine, flagged gap, not papered over | Governance Agent | **Open — real, mechanical work not yet done** |
| **New Walkthrough script** *(Walkthrough G)* | Written and **partially run** — Home Dashboard, APEX charts, and the Reviewer's Workspace were all walked through live across two passes; the specific "enter a module, decide, return Home, re-enter" sequence Session 61 itself calls for has not yet been run | Governance Agent wrote it; Project Principal ran what's been run so far | **Partially done — one specific sequence remains, now higher-priority than when this row was last written** |
| **Rehearsal** | Full demo script dry-run, including the role-based access story and the Reviewer's Workspace | Project Principal + Governance Agent | Documentation-currency pass complete; the remaining Walkthrough sequence run clean |
...
|---|---|
| 27–61 | All done |
| Rehearsal | Documentation-currency pass complete (done); the remaining Walkthrough sequence (Session 61's Home-return test) run clean |
...
| **`HUMAN_DECISION` "context depth" field** *(still open, unchanged)* | Nothing in the current build sequence — a real, concrete proposal from `docs/22` §6, not yet a governance decision | Would let every future decision honestly record whether it was made from curated context alone or after going deeper — real audit value, low cost, worth a deliberate yes/no rather than silent adoption or silent dismissal. **Adjacent but distinct from GD-28's Activity View (Session 58), which now lets a reviewer see their own decision history but does not itself record "context depth" for any single decision — both remain real, separate proposals.** |
| **`docs/NN` spec placement, as its own standing risk** | Has caused real friction three times now — `docs/20` (Session 44), `docs/25` (Session 53), and a related pattern: two consecutive sessions (60, 61) each got a small claim about the Session 55 files' repo location wrong, in two different ways, neither ever actually checking `git status` before writing it down | Worth a standing habit, not a one-off fix: verify a claim about the repository's *state*, not just its code, with the same rigor as a technical claim — `ls`/`git status` before writing either kind down |
| **An ordering dependency between two unrelated-looking fixes can be real** *(new this version)* | Session 60's assessment found that a completely separate navigation fix (Home-return) had a latent, previously-invisible dependency on the session-state-resurrection fix being done first — confirmed to hold exactly as predicted once Session 61 actually built both | For any two findings touching the same subsystem even loosely, ask explicitly: does fixing one change an assumption the other quietly depends on? Not just "can these be scheduled in either order." |
| **The shared Anthropic API key architecture (D4-6)** *(new this version)* | Seven modules share an identical pattern reading `VITE_ANTHROPIC_API_KEY`, compiled into the client bundle by Vite's own design. No real key exists anywhere today | Not urgent for a synthetic-data demo. A genuine pre-production gate — needs a real decision (server-side proxy vs. runtime injection) before any real key is ever configured. The single finding from the whole Session 60 assessment most directly relevant to eventually handling real, sensitive data. |
| **Module health dots wired to nothing (D3-6)** *(new this version)* | `pollAll()`/`startHealthPolling()` exist, are never called; every sidebar health dot permanently shows "unknown" | Decide: finish wiring, or remove the dead code and the dots entirely |
| **ARIA / APEX / FLOWPATH banner overclaims enforcement (D4-5)** *(new this version; scope corrected in place July 24, 2026 — see `docs/30` §3)* | Three modules state CUI+ processing "is blocked and logged" — true of the platform's real intake paths, not of these modules' own inputs specifically. VIGIL was named in error and carries no such banner | Decide: soften the wording, or extend real enforcement to match what it already claims — across three modules, not one |
| **LENS's incomplete governance-explanation source documents (D4-9)** *(new this version)* | Six required, two exist, neither is even on the registry's own required list | A content-authoring task, not a code task — needs someone to write the missing documents |
...
| **A single-role access gate can silently leave real users with zero access, invisibly, for months** *(confirmed real, v3.4)* | Confirmed real, not hypothetical: SCRIBE was built and walkthrough-tested entirely under SYSTEM_ADMIN, and nothing until the DEV persona toggle existed would have revealed that PROGRAM_MANAGER — the role SCRIBE's own spec names as its user — could not open it at all. Mitigation: the persona toggle now exists and covers all 8 roles in code, all eleven modules' actual gates independently re-verified against the access matrix in Session 60's assessment, matching exactly. |
| **A component's state can silently reset on remount, undoing a decision that was supposed to be permanent** *(new this version, confirmed real six times)* | First found and fixed as WG-13 (VIGIL approvals, Session 54), then SCRIBE (Session 55). Session 60's comprehensive assessment found the identical bug class in four more places — including one, ARIA's Gate 3/4, where the UI's own language explicitly promised permanence the code didn't keep. All six now share one proven, named pattern (`AGENT_REFERENCE.md` v3.3), closed in Session 61. Mitigation going forward: any new component holding decision state that should survive a remount defaults to this pattern, not a fresh `useState`. |
| **An external tool's shared configuration can silently leak content across unrelated projects** *(new this version)* | Confirmed real and recurring: a Claude Code memory folder scoped to the home directory, not to this project specifically, bled an unrelated project's content into SOVEREIGN session close artifacts three separate times (Sessions 44, 49, 58) before the pattern was actually traced to its root cause and cleaned at the source, rather than just deleted each time it recurred. Mitigation: the specific contamination was removed; the underlying tool configuration was deliberately left as-is, per direct Project Principal decision to keep the feature rather than disable it — worth re-checking if it ever recurs a fourth time. |
...
- [x] ~~Requested a full documentation-currency pass~~ — **COMPLETE this version**: Agent-to-Agent
  Briefing, Integration Brief, New Conversation Handoff, `docs/18`, this document,
  `AGENT_REFERENCE.md`, `docs/22`/`28`/`29`/`30`, Role Access Matrix all rewritten or updated
- [x] ~~Resolve the WG-11/WG-8/WG-7 program-model question~~ — DECIDED, `docs/29`
- [x] ~~Approve GD-28 (Logger read exposure + Activity View)~~ — APPROVED, Session 58
- [x] ~~Order the Session 60 comprehensive assessment~~ — COMPLETE
- [x] ~~Order the resurrection-family + navigation fix session~~ — COMPLETE, Session 61
- [x] ~~Decline disabling Claude Code's memory feature; approve the targeted cleanup instead~~ — DECIDED, Session 59
- [ ] **Run the remaining Walkthrough sequence** — Session 61's own specific ask: enter a
  module, decide an item, return Home via the breadcrumb, re-enter the module. The one thing
  left that genuinely needs a human in a browser.
- [ ] **Decide D4-6** — the shared API key architecture, before any real key is ever configured
- [ ] **Decide WG-6's real (non-cosmetic) resolution** — Session 59 padded the demo data only
- [ ] **Decide D3-6** (health dots), **D4-5** (banner wording vs. real enforcement), **D4-9**
  (LENS source documents) — three small, independent, non-blocking decisions
- [ ] **Provide the SBOM Registry's last real content** — still not merged past Session 53;
  eight real session updates (54–61) exist and need folding in
...
**Governance Agent:**
- [x] ~~Reconcile Sessions 35 through 61's closes~~ — DONE, each independently re-verified against the actual repo, not taken from any recap
- [x] ~~Write the `ProgramStatusSurface` spec (`docs/20`)~~ — DONE, placed
- [x] ~~Write the `WorkQueueSurface` spec (`docs/21`)~~ — DONE, placed
- [x] ~~Write `docs/22`, the informed-decision-making design philosophy~~ — DONE, placed, updated July 24
...
- [x] ~~Write `docs/26` (Execution Monitoring vision), `docs/27` (execution-governance response), `docs/28` (Logger read path), `docs/29` (WG-11/WG-7/WG-14 decisions), `docs/30` (Session 60 assessment action plan)~~ — DONE, all placed, `docs/28`–`30` updated July 24 to record what actually shipped
- [x] ~~Produce the July 21 System Prompt refresh (v35)~~ — DONE, since superseded by v36 and v37
- [x] ~~Rewrite the Agent-to-Agent Briefing~~ — DONE, July 24
- [x] ~~Rewrite the Platform Integration Brief~~ — DONE, v1.49, July 24
- [x] ~~Rewrite the New Conversation Handoff~~ — DONE, v7, July 24
- [x] ~~Read and assess `docs/18`~~ — DONE; content unchanged, a currency addendum added July 24
- [x] ~~Write a new Walkthrough script~~ — DONE, Walkthrough G, partially run (see Part III)
- [x] ~~Update the Role Access Matrix for `module-workspace`'s own gating, and later the Activity tab~~ — DONE, both passes
- [ ] **Reconstruct or receive the SBOM Registry (v1.41 → v1.42)** — genuinely not yet done; eight
  real session updates (54–61) exist, need merging. The one real documentation gap left.
- [ ] **Retire or convert `SOVEREIGN_Design_Recommendations_20260718.md`** — several recommendations are now shipped
- [ ] **Build a consolidated shell-surface reference** — DONE, `SOVEREIGN_Shell_Surface_Reference_20260721.md`, still accurate (GD-28 widened an existing export, added no seventh surface)
...
**Build Agent:**
- [x] ~~Sessions 27 through 61~~ — all complete
...
- [x] ~~Sessions 54–56 — Walkthrough G build findings + test-coverage/audit-scope remediation~~ — done
- [x] ~~Session 57 — native PPBE Program Detail view + Module Orientation live status~~ — done
- [x] ~~Session 58 — GD-28, Activity & Decisions tab~~ — done, one naming-convention slip corrected
- [x] ~~Session 59 — WG-6 demo padding, decision-note reason codes, memory-contamination cleanup~~ — done
- [x] ~~Session 60 — comprehensive end-to-end R/E/S assessment~~ — done, findings only, nothing fixed by design
- [x] ~~Session 61 — the full resurrection family + Home-return navigation, all seven deliverables~~ — done
...
- [ ] **D4-6** — decide and, once decided, implement the real API key architecture before any real key exists
- [ ] **D3-6, D4-5, D4-9, D3-8, D3-10** — small, independent, non-blocking (see Part VI)
- [ ] **Two new standing rules, added System Prompt v35, held through every session since:** do not
  author/edit `docs/NN` spec files (Session 52's `docs/24` incident); always verify a spec is
  actually in the repo before a session that depends on it (Sessions 44 and 53 both hit this)
- [ ] No build session currently authorized — awaiting the Project Principal's next real decision
  (the Walkthrough sequence, or one of D4-6/WG-6/D3-6/D4-5/D4-9)
...
| **July 24, 2026** | **v3.7 — the largest single update in this document's history.** Eight sessions closed (54–61). `docs/29` resolved the largest open architecture question from v3.6 (APEX's World Model vs. PPBE's synthetic programs — decided not to force-merge, built a native PPBE Program Detail view instead, Session 57). GD-28 shipped (Session 58) — the Logger's dormant read method finally exposed, a real Activity & Decisions tab built. Session 60 ran the platform's first comprehensive, code-level R/E/S assessment across all eleven modules — explicitly not a Walkthrough substitute — and found the session-state-resurrection bug class in four more places plus a real, confirmed ordering dependency with any future Home-return navigation work. **Session 61 closed all of it**, in the required order, in the largest single session this plan has ever recorded. **The documentation-currency pass v3.6 called "in progress" is now complete** — this document, the Integration Brief, the Agent-to-Agent Briefing, the New Conversation Handoff, `AGENT_REFERENCE.md`, four `docs/NN` files, and the Role Access Matrix were all rewritten or updated the same day. **The SBOM Registry remains the one real, flagged, un-closed documentation gap** — eight session updates exist, not yet merged. **The live Walkthrough repeat pass is now the platform's single oldest and most load-bearing open item**, carrying a specific new reason it matters: Session 61's own close names the exact sequence that would confirm its two biggest fixes actually work together, and that sequence has never been run in a real browser. | Sessions 54-61, one governance conversation (`docs/29`), one comprehensive platform-wide assessment, and a full documentation-currency pass covering every core governance document |
...
**A related but distinct capability shipped since this part was written —
worth noting here so the two aren't conflated:** GD-28 (Session 58) gave
reviewers a real Activity & Decisions view of their own past decisions,
session-scoped. That's genuinely useful and worth mentioning to a CTO
alongside this philosophy, but it is not the "context depth" field above —
one shows *what* was decided; the other, still unbuilt, would record *how
much context* the reviewer had when deciding it. Both real, both separate.

---

*SOVEREIGN Platform — Strategic Plan to CTO Demo · v3.7 · July 24, 2026*
*Pre-Decisional · Internal Working Document*
*File to: git `docs/`*
*Companion documents: `SOVEREIGN_EndToEnd_Assessment_20260719.md`;
`SOVEREIGN_Platform_EndToEnd_Assessment_20260723.md`;
`docs/22_Informed_Decision_Making.md`; `docs/23_Reviewers_Workspace_v1.md`;
`docs/25_Cross_Module_Navigation_Primitive.md`; `docs/26` through `docs/30`*

#!/bin/bash
# SOVEREIGN — Session 124 Context Gather Script
...
OUT="/tmp/sovereign_session124_context.txt"
> "$OUT"
...
echo "SOVEREIGN — Session 124 context gather   $(date '+%Y-%m-%d %H:%M')"
echo "HEAD: $(git rev-parse --short HEAD)"
echo "Expected prior close: Session 123, terminal HEAD f651f2c — confirm below."
echo
...
if command -v pbcopy >/dev/null 2>&1; then
  ...
  echo "then paste the Session 124 opening prompt (F-51, six-mode schema"
  echo "validation fix) as the second."
else

#!/bin/bash
# SOVEREIGN — Session 125 Context Gather Script
...
OUT="/tmp/sovereign_session125_context.txt"
> "$OUT"
...
echo "SOVEREIGN — Session 125 context gather   $(date '+%Y-%m-%d %H:%M')"
echo "HEAD: $(git rev-parse --short HEAD)"
echo "Expected prior close: Session 124, terminal HEAD 2af445e — confirm below."
echo
...
if command -v pbcopy >/dev/null 2>&1; then
  ...
  echo "then paste the Session 125 opening prompt (F-51 extension to TT"
  echo "Draft and PPBE Exhibit validators) as the second."
else
